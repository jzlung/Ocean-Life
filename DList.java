...
//THIS IMPLEMENTATION IS DESIGNED TO WORK WITH INT[] ARRAYS FOR PROJECT 1.
...
public class DList {
  ...
  /**
   *  DList() constructor for a one-node DList2.
   */
  ...
  /**
   *  DList() constructor for a two-node DList2.
   */
   ...
   */
    public void insertFront(int type, int length) {
      DListNode x = new DListNode(type, length);
      if(size==0){
	  head.next=x;
	  head.prev=x;
	  x.next=head;
    ...
    public void insertEnd(int type, int length) {
	if(size==0){
	    insertFront(type,length);
	} else{
	DListNode insertThis = new DListNode(type, length);
	DListNode lastNode = head.prev;
	lastNode.next = insertThis;
	insertThis.next = head;
	insertThis.prev=lastNode;
	head.prev=insertThis;
	size++;
	}
}

    public void insertEnd(DListNode x){
	if(size==0){
	    head.next = x;
	    head.prev = x;
	    x.prev = head;
